...
| Последний коммит в `main` | **`0d118e9`** — `fix(payments): harden bePaid invoice idempotency` (v7.0.82) |
...
> v7.0.82 запушен, но **НЕ деплоился** на production-сервер. Деплой — только по команде владельца.
